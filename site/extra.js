(()=>{
  ...
  function patientAppts(patient,d){
    const phone=String(patient.phone||'').replace(/\D/g,''), email=String(patient.email||'').toLowerCase();
    return (d.appointments||[]).filter(a=>a.patient_id===patient.id || (phone&&String(a.patient_phone||'').replace(/\D/g,'')===phone) || (email&&String(a.patient_email||'').toLowerCase()===email));
  }
  ...
  function renderPatients(){
    ...
    el.innerHTML=rows.map(p=>{
      ...
      const latest=aps[0];
      return `<tr><td><div class="patient-cell"><b>${safe(ini(p.full_name))}</b><span><strong>${safe(p.full_name||'Unnamed patient')}</strong><small>${safe(p.phone||'No phone')}</small></span></div></td><td>${safe(p.email||'—')}</td><td><strong>${aps.length}</strong><small>${latest?`Latest: ${safe(when(latest.starts_at))}`:'No appointment yet'}</small></td><td>${latest?badge(latest.status):'<span class="extra-muted">—</span>'}</td><td><button class="row-button" data-book-patient="${p.id}">Book appointment →</button></td></tr>`;
    }).join('');
  ...
  function renderCalls(){
    ...
    el.innerHTML=calls.length?calls.map(c=>{
      const p=linkedPatient(c,d),a=linkedAppointment(c,d);
      const who=p?.full_name||(c.direction==='inbound'?c.caller:c.callee)||'Unknown caller';
      return `<article class="call-card"><div class="call-card-top"><div class="call-who"><span class="call-avatar">${safe(ini(who))}</span><div><small>${safe((c.direction||'call').toUpperCase())} CALL</small><h3>${safe(who)}</h3><p>${safe(c.caller||'—')} → ${safe(c.callee||'—')}</p></div></div>${badge(c.status)}</div><div class="call-meta"><div><small>Time</small><strong>${safe(when(c.created_at))}</strong></div><div><small>Duration</small><strong>${duration(c.duration_seconds)}</strong></div><div><small>Appointment outcome</small><strong>${a?safe(nice(a.status)):'Not linked'}</strong></div><div><small>Provider call ID</small><strong>${safe(c.provider_call_id||'—')}</strong></div></div><div class="call-summary"><small>AI SUMMARY</small><p>${safe(c.summary||'No call summary has been recorded yet.')}</p></div><details class="transcript"><summary>View transcript</summary><pre>${safe(c.transcript||'No transcript available.')}</pre></details>${a?`<button class="row-button call-open-appt" data-call-appt="${a.id}">Open linked appointment →</button>`:''}</article>`;
  ...
  setInterval(()=>{setupNav();bindFilters();const d=currentData();if(!d)return;const stamp=String(d.generated_at||'')+':'+(d.patients||[]).length+':'+(d.calls||[]).length;if(stamp!==lastStamp){lastStamp=stamp;renderExtras()}},350);
})();